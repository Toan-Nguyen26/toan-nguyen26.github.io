...
- 1039 files, 98 MB total; largest single file 9.6 MB
...
1. Pick a subfolder of the Pages repo, e.g. `c3vd/`, so the viewer lives at
   `https://<user>.github.io/c3vd/`.
2. Copy **the contents of this folder** into it (so `c3vd/index.html` exists).
...
5. **Verify on the live URL** (not locally):
   - four segment buttons in the top-left panel, none with a dashed outline
   - the browser console shows no errors
   - clicking the wall opens a panel with a real colonoscope frame
   - switching between *Clarity* and *Seen or not* recolours the model

## Updating later

Rebuild with `build_web.py`, rerun `package_site.py`, and **replace the whole folder**
...
C3VD data is CC BY-NC-SA 4.0. Keep the credit panel in the page, keep `README.md`,
and don't use it commercially. If you link to the viewer from elsewhere on the site,
credit C3VD (Bobrow et al., 2023) next to the link.
